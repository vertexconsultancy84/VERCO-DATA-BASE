...
const getPropertyIcon = (propertyType: string) => {
  switch (propertyType) {
    case 'house-for-sale': return <Home className="w-5 h-5" />;
    case 'land-for-building': return <Building className="w-5 h-5" />;
...
const getPropertyLabel = (propertyType: string) => {
  switch (propertyType) {
    case 'house-for-sale': return 'House for Sale';
    case 'land-for-building': return 'Land for Building (IBIBANZA)';
...
const propertyTypes = [
  ...
  { value: 'house-for-sale', label: 'Houses for Sale', icon: <Home className="w-4 h-4" /> },
  { value: 'land-for-building', label: 'Land for Building (IBIBANZA)', icon: <Building className="w-4 h-4" /> },
];
...
export default function ForSalePage() {
  ...
  const fetchProducts = async () => {
    try {
      console.log('=== FOR SALE PAGE DEBUG ===');
      ...
      if (result.success) {
        ...
        console.log('Sample products:', result.data.slice(0, 3).map((p: Product) => ({
          id: p.id,
          title: p.title,
          category: p.category,
        ...
        // Filter for sale properties (RealEstate category and for-sale subcategory)
        const saleProducts = result.data.filter((product: Product) => 
        ...
        console.log('Filtered sample:', saleProducts.slice(0, 3).map((p: Product) => ({
          id: p.id,
          title: p.title,
          category: p.category,
          subcategory: p.subcategory,
        ...
        console.log('Products set in state:', saleProducts.length);
      } else {
  ...
  const getPropertyCounts = () => {
    const counts = {
      all: products.length,
      'house-for-sale': products.filter(p => p.propertyType === 'house-for-sale').length,
      'land-for-building': products.filter(p => p.propertyType === 'land-for-building').length,
    };
    return counts;
  };
      ...
      <div className="bg-gradient-to-r from-[#023E4A] to-[#0097A7] text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-12">
          <div className="text-center mb-6">
            ...
            <p className="text-xl opacity-90">Find your perfect property - houses for sale and land for building (IBIBANZA)</p>
          </div>
          <div className="text-center">
            <Link href="/">
                ...
                >
                  ...
                  <Badge variant="secondary" className="ml-1">
                    {type.value === 'all' ? getPropertyCounts().all : 
                     type.value === 'house-for-sale' ? getPropertyCounts()['house-for-sale'] :
                     getPropertyCounts()['land-for-building']}
                  </Badge>
                </Button>
              ))}
            </div>
            {selectedFilter !== 'all' && (
              <div className="mt-4 flex items-center justify-between">
                <p className="text-sm text-gray-600">
                  Showing {filteredProducts.length} {getPropertyLabel(selectedFilter).toLowerCase()}
          ...
          <div className="text-center py-12">
            <div className="bg-white rounded-lg shadow-sm p-8">
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                {selectedFilter === 'all' ? 'No Properties for Sale Available' : `No ${getPropertyLabel(selectedFilter).toLowerCase()}s Available`}
              </h3>
              <p className="text-gray-600 mb-6">
                {selectedFilter === 'all' 
                  ? 'There are currently no properties for sale. Please check back later.'
                  : `There are currently no ${getPropertyLabel(selectedFilter).toLowerCase()}s for sale. Try other property types or check back later.`
                }
              </p>
                    ...
                    {images.length > 0 ? (
                      <div className="relative h-64">
                        ...
                        <div className="absolute top-2 right-2 flex gap-2">
                          <Badge className="bg-emerald-600">For Sale</Badge>
