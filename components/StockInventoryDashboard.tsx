...
export default function StockInventoryDashboard({ productsSlot }: { productsSlot?: React.ReactNode } = {}) {
  ...
  const togglePublish = async (id: string, current: boolean) => {
    ...
    await patchFinishedProductStockRecord(id, { published: !current });
    setPublishing(null);
  };

  // total IN / OUT per record id, from recorded movements
  ...
  // finished-products inventory list
  const items = useMemo<InvItem[]>(() => {
    return finished.map((r) => {
      ...
      return {
        ...
        published: r.published, low: current <= 0 || (threshold > 0 && current <= threshold),
      };
    });
  }, [finished, moveTotals]);
              ...
              <table className="w-full text-xs border-collapse min-w-[640px]">
                <thead>
                  <tr className="bg-gray-50 text-gray-500 border-b border-gray-200">
                    {["Product", "Initial", "Current Stock", "Unit", "Low Stock", "Reserved", "Status"].map((h) => (
                ...
                <tbody>
                  {filtered.map((it) => (
                    <tr key={it.id} className="border-b border-gray-50 hover:bg-[#0097A7]/5 transition-colors">
                      ...
                      <td className="px-4 py-2.5">
                        <span className={`font-bold ${it.low ? "text-red-600" : "text-emerald-600"}`}>
                          {it.current.toLocaleString()}
                        </span>
                        {it.low && <span className="ml-1.5 inline-block w-1.5 h-1.5 rounded-full bg-red-500 align-middle" title="Low stock" />}
                      </td>
                      <td className="px-4 py-2.5">
                        <span className="bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{it.unit}</span>
                      </td>
                      <td className="px-4 py-2.5 text-gray-500">
                        {it.threshold > 0 ? Math.round(it.threshold).toLocaleString() : "—"}
                      </td>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`font-bold ${lowStockCount > 0 ? "text-amber-600" : "text-gray-400"}`}>{lowStockCount}</span>
              </div>
            </div>
            <Link
              href="/industry/finished-products"
            ...
            >
              <Truck className="w-4 h-4" /> Restock{lowStockCount > 0 ? ` (${lowStockCount})` : ""}
            </Link>
          </div>
